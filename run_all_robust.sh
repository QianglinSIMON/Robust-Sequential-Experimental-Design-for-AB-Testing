...
# 1) Sequential_DP_robust.py
# 2) Training_Q_last_net_robust_nonscal.py
# 3) Training_Q_net_iteration_robust_nonscal.py
...
# DP_seq hyperparameter used by Sequential_DP_robust
...
########################################
echo "[$(date)] Step 2/3: Training_Q_last_net_robust_nonscal.py"
echo "--------------------------------------------------------"

STEP2_LOG="${LOG_DIR}/${LOG_PREFIX_STEP2}_${TYPE_OBJ}_nexp${N_EXP}_gpu${GPU_ID}_${TS}.stdout.log"

python Training_Q_last_net_robust_nonscal.py \
  --gpu "${GPU_ID}" \
  --n_exp "${N_EXP}" \
  --type_obj "${TYPE_OBJ}" \
  --epochs "${LAST_EPOCHS}" \
...
########################################
echo "[$(date)] Step 3/3: Training_Q_net_iteration_robust_nonscal.py"
echo "--------------------------------------------------------"

STEP3_LOG="${LOG_DIR}/${LOG_PREFIX_STEP3}_${TYPE_OBJ}_nexp${N_EXP}_gpu${GPU_ID}_${TS}.stdout.log"

python Training_Q_net_iteration_robust_nonscal.py \
  --gpu "${GPU_ID}" \
  --n_exp "${N_EXP}" \
  --obj_type "${TYPE_OBJ}" \
  --B_share "${B_SHARE}" \
